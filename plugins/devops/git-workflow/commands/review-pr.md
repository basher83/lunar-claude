...
**Constraints:**
- Do NOT use `run_in_background: true` on any Task call. All agents run in foreground.
...
- CONTEXT-FIRST — understand author intent before reviewing.
...
Task(subagent_type: "general-purpose", model: "haiku",
     prompt: "Get PR metadata using gh CLI:
              gh pr view $ARGUMENTS --json number,title,body,commits,files,headRefName,baseRefName,labels,author
...
Task(subagent_type: "general-purpose", model: "haiku",
     prompt: "Find all CLAUDE.md files in the repository being reviewed.
              ...
              - Get local path by finding it: fd -t d -d 3 '<repo-name>' ~ 2>/dev/null | head -1
...
Task(subagent_type: "general-purpose", model: "haiku",
     prompt: "Extract ticket references from PR metadata:
              ...
              2. Parse for ticket patterns:
                 ...
                 - Linear: ABC-123
              3. If GitHub issues found, fetch with: gh issue view [number] --json title,body
...
Task(subagent_type: "git-workflow:code-reviewer", model: "sonnet",
...
Task(subagent_type: "git-workflow:structural-reviewer", model: "sonnet",
