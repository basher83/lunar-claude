...
def print_validation_result(
    name: str, is_valid: bool, message: str, details: dict, verbose: bool = False
):
...
def main():
    ...
    parser.add_argument("name", nargs="?", help="DNS name to validate")
    parser.add_argument("--file", help="File containing DNS names (one per line)")
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Show detailed component breakdown"
    )
    parser.add_argument(
        "--check-format", action="store_true", help="Only check format, don't suggest corrections"
    )
    ...
    if args.file:
        ...
        if results["valid"]:
            ...
            for name, details in results["valid"]:
                ...
                if args.verbose:
                    print(
                        f"      Service: {details['service']}, Number: {details['number']}, Purpose: {details['purpose']}"
                    )
    ...
    if not is_valid and not args.check_format:
        ...
        print("   - Number must be 2 digits: docker-1-app → docker-01-app")
        print("   - Pattern: <service>-<NN>-<purpose>.<domain>")
        print("   - Example: docker-01-nexus.spaceships.work")
