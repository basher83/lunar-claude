...
5. Enable Amazon Bedrock and request access to your target Claude models
6. Create an IAM OIDC provider for GitLab if not already present
7. Create an IAM role trusted by the GitLab OIDC provider, restricted to your project and protected refs
8. Attach least-privilege permissions for Bedrock invoke APIs
...
4. Enable IAM Credentials API, STS API, and Vertex AI API
5. Create a Workload Identity Pool and provider for GitLab OIDC
6. Create a dedicated service account with Vertex AI roles
7. Grant the WIF principal permission to impersonate the service account
