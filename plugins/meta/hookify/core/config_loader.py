...
def load_rules(event: str | None = None) -> list[Rule]:
    ...
    for file_path in files:
        try:
            ...
            # Filter by event
            if event:
                # Include only 'all' or matching event
                if rule.event != "all" and rule.event != event:
                    continue
            else:
                # When event is None (unknown tool type), only include 'all' rules
                # This prevents prompt/stop/bash/file rules from triggering on unrelated tools
                if rule.event != "all":
                    continue
